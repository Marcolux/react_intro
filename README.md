...
React will work with any back-end language, but for our in-class examples we will be using Express with a PostGres database for the models and controllers.
...
In your terminal, navigate to the directory where you would like to create your React app. And then run the following command.
...
`create-react-app` is a package that will handle the details of setting up a React project for us. It will install the necessary dependencies, and set up a server to serve our front end files to the browser. It will also watch our files for any changes and automatically update the browser when we save a file, also called "hot reloading".

An alternative to running `npx create-react-app my_first_react_app` is to run `yarn create react-app my_first_react_app` . Notice one approach uses "create-react-app" and the other uses "create react-app" (it has one fewer dash).
...
Before we wipe out the default contents of App.js & App.css, let's look them over. Open your new React app up in your code editor and take a look inside. You'll see a lot of files in here. Many of them are not important for us to uderstand at the moment. We'll just focus on the core pieces.
